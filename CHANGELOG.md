...
- **Terminal colors**: the game is no longer monochrome. A hand-rolled,
  ...
  green → yellow → red as they drain. The render engine also colors systematic
  patterns automatically, so every message gets them with no call-site markup:
  quoted speech (dim), levels `lv.5` (cyan), percentages (yellow),
  `HP:`/`SP:`/`QP:` stat tokens (green/yellow/magenta), `Round N`/`Day N`
  headers (bold), healing `+N HP` (green), money amounts (`N coins`, `N-coin`,
  yellow) and exp amounts (`N exp`, green). All menus show cyan option keys and
  white option text; the style-selection menu adds grey short descriptions,
  `yes`/`no` prompts are green/red, page navigation is grey, press-key prompts
  are grey, the fight-distance visualization shifts red → cyan as range grows,
  and move-bonus stars are yellow. All alignment code (`align_text`,
