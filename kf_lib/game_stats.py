...
class StatGen(object):
    ...
    def get_full_report(self):
        g = self.game
        labels = self.get_player_data(g.current_player, labels_only=True)
        report = [[lab] for lab in labels]
        for p in g.players:
            data = self.get_player_data(p, data_only=True)
    ...
    def get_player_data(self, p, labels_only=False, data_only=False):
        gs = p.get_stat
        ...
        full = [('\n*GENERAL*', ''),
                ...
                ('Level (Exp.)', '{} ({})'.format(p.level, p.exp)),
                ('Strength', '{}'.format(p.get_att_str('strength'))),
                ('Agility', '{}'.format(p.get_att_str('agility'))),
                ('Speed', '{}'.format(p.get_att_str('speed'))),
                ('Health', '{}'.format(p.get_att_str('health'))),
                ('Techs', len(p.techs)),
                ('Moves', len(p.moves)),

                ('\n*FIGHTING*', ''),
                ('Fights ({})'.format(ft), '{} ({}%)'.format(nf, fpcnt)),
